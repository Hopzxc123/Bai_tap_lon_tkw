...
}// Thêm hàm để lấy thông tin đơn hàng
function getOrderDetails() {
    ...
    cartItems.forEach(item => {
        ...
        itemDetail.innerHTML = `<p>${item.name} - ${item.price.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })} x ${item.quantity}</p>`;
        orderDetailsElement.appendChild(itemDetail);
    });
    // Cập nhật tổng tiền và các thông tin liên quan
    updateTotalPrice();
}
...
function validateForm() {
    ...
    if (isValid) {
        ...
        // Chuyển hướng về trang order.html
        window.location.href = "order.html";
    }
}
...
function addToCart(name, price) { // THÊM
    const existingItem = cartItems.find(item => item.name === name);
    if (existingItem) {
        existingItem.quantity += 1; // Nếu sản phẩm đã có trong giỏ hàng, tăng số lượng lên 1
    } else {
        cartItems.push({ name, price, quantity: 1 }); // Nếu không, thêm sản phẩm mới
    }
    saveCartToLocalStorage(); // Lưu giỏ hàng sau khi thêm sản phẩm
    renderCartItems(); // Cập nhật giỏ hàng
    updateTotalPrice(); // Cập nhật tổng tiền
    showToast(`${name} đã được thêm vào giỏ hàng!`); // THÊM thông báo
} // THÊM
function clearCart() {
    ...
    document.getElementById("shippingFee").innerText = "0đ";
    document.getElementById("discount").innerText = "0 VNĐ";
    document.getElementById("promo-amount").innerText = "0 VNĐ";
